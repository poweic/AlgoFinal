...
float GiveDimMixD(MixPDF *mp, int k, int priortype){
   ...
   if (priortype == 1){  /* MMI Prior */
      ...
      ma1=(MuAcc*)GetHook(mp->mean);
      ma2=ma1+1;
      va1=(VaAcc*)GetHook(mp->cov.var);
      va2=va1+1;
   } else
...
static void _FixHMMForICrit(float Tau, Boolean THREEACCS){
   ...
   while(GoNextMix(&hss,FALSE)){
      ...
      */
      ...
      ma_dst = (MuAcc*) GetHook(mp->mean);
      va_dst = (VaAcc*) GetHook(mp->cov.var);
...
void FloorVars(HMMSet *hset1, int s){
  ...
  if(!(hset1->hsKind==PLAINHS || hset1->hsKind==SHAREDHS)){
     ...
     varray = (float**) New(&gstack, sizeof(float*) * (vsize+1));
     for(i=1;i<=vsize;i++) varray[i] = (float*) New(&gstack, sizeof(float) * M);
