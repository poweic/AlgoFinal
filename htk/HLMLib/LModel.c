...
void StoreFEA(FLEntry *fe, MemHeap *heap)
{
   ...
   if (fe->nfe==0) {
      ...
      fe->fea = (_FLEntry*) memcpy(febuf, fe->fea, fe->nfe*sizeof(FLEntry));
   }
}
...
void StoreSEA(FLEntry *fe, MemHeap *heap)
{
   ...
   if (fe->nse==0) {
      ...
      fe->sea = (_SMEntry*) memcpy(sebuf, fe->sea, fe->nse*sizeof(SMEntry));
   }
}
...
void ReadHeaderInfo(Source *src, BackOffLM *lm, char *line1)
{
   ...
   lm->probType = LMProbType(LMP_FLOAT|LMP_LOG);
...
static void ReadClassCounts(Source *src, int nWords, BackOffLM *lm)
{
   ...
   for (i=0; i<nWords; i++) {
      ...
      *ptr2 = '\0';
      ...
      wordProb = (WordProb*) New(&gcheap, sizeof(WordProb));
      wordProb->_class = nid2;
...
static void CountClassTotals(BackOffLM *lm)
{
   ...
   lm->totals = (int*) New(&gcheap, lm->vocSize * sizeof(int));
   ...
   for (i=0; i<(lm->classW); i++) {
      ...
      class_id = (int)(((WordProb*)(lm->classBM[i+1]->ptr))->_class->ptr);
			 ...
			 LMProbType tgtPType, MemHeap *heap)
{
   ...
   if (strncmp(lnBuf, "Class-based LM", 14)==0) {
      ...
      if (lm->classCounts) {
         ...
         lm->word = (int*) New(&gcheap, nWords * sizeof(int));
         for (j=0; j<nWords; j++) {
            lm->word[j] = 0;
         }
      }
   ...
   lm->probType = LMProbType(lm->probType & tgtPType);
...
float GetNGramProb(BackOffLM *lm, NameId *words, int nSize)
{
   ...
   */
   ...
   if (lm->classLM) {
      ...
      for (i=0; i<nSize; i++) {
	 words[i] = ((WordProb*)(words[i]->ptr))->_class;
      }
   }
