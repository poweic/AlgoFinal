...
int main(int argc, char *argv[])
{
   ...
   while (NextArg() == SWITCHARG) {
      ...
      switch(s[0]){
	   ...
	   dEntry= (dictList*) New(&gcheap,sizeof(dictList));
   ...
   if (wlistFN!=NULL) {
	...
	fflush(stdout);
      ...
      lm = LoadLangModel(srcFN,&wlist,1.0, LMProbType(LMP_FLOAT|LMP_COUNT),&langHeap);
   } else {
      voc = NULL;
      lm = LoadLangModel(srcFN,NULL,1.0, LMProbType(LMP_FLOAT|LMP_COUNT),&langHeap);
   }
   if (trace&T_TOP) {
