...
void set_output_prefix(char *name)
{
   ...
   export_prefix = (char*) New(&global_stack, strlen(name)+1);
...
void classes_init(int numb_words)
{
   ...
   class_sort = (UInt*) CNew(&global_stack, W * sizeof(UInt));
   clSum = (int*) CNew(&global_stack, N * sizeof(int));
   tmp_c1 = (int*) CNew(&global_stack, N * sizeof(int));
   tmp_c2 = (int*) CNew(&global_stack, N * sizeof(int));
   tmp_c3 = (int*) CNew(&global_stack, N * sizeof(int));
   tmp_c4 = (int*) CNew(&global_stack, N * sizeof(int));
   tmp_sum1 = (int*) CNew(&global_stack, N * sizeof(int));
   tmp_sum2 = (int*) CNew(&global_stack, N * sizeof(int));
   mlv = (double*) CNew(&global_stack, N * sizeof(double));
   sort_uni = (int*) CNew(&global_stack, W * sizeof(int));
   if (!clMemb)
      clMemb = (int*) CNew(&global_stack, W * sizeof(int)); /* May have been setup by existing map */
   clCnt = (int**) CNew(&global_stack, N * sizeof(int *));
   for (i=0; i<N; i++) {
      clCnt[i] = (int*) CNew(&global_stack, N * sizeof(int));
   }
   if (trace & T_MEM) {
   ...
   bipair = (int*) CNew(&global_stack, W * sizeof(int));
   for (i=0; i<W; i++) {
      bipair[i] = 0;
...
#define max_line_len 500 
   ...
   W = numb_words;
   clMemb = (int*) CNew(&global_stack, W * sizeof(int));
...
{
   max_words = words;
   uni = (unigram*) CNew(&global_stack, words * sizeof(unigram));
...
void bigram_add(NGram ng, int count)
{
   ...
   if (ng[0] == last_word) {
      ...
      if (store_idx >= curr_bistore_size) {
         ...
         store = (bi_count*) realloc(store, curr_bistore_size*sizeof(bi_count));
      }
   ...
   ptr = (bi_count*) get_space(space_used);
...
void bigram_added_all(void)
{
   ...
   for (i=0; i<max_words; i++) {
      backward[i].bi = (bi_count*) get_space(backward[i].size * sizeof(bi_count));
...
{
   max_words = words;
   forward = (bigrams*)  CNew(&global_stack, words * sizeof(bigrams));
   backward = (bigrams*)  CNew(&global_stack, words * sizeof(bigrams));
   if (trace & T_MEM) {
   ...
   store = (bi_count*) calloc(initial_bigram_buffer, sizeof(bi_count));
   if (trace & T_MEM) {
