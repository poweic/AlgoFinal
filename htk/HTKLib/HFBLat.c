...
static void SetCorrectness(FBLatInfo *fbInfo, Lattice *numLat){  
   ...
   correctArc= (CorrectArcList**) New(&fbInfo->tempStack, sizeof(CorrectArcList*)*(fbInfo->T+1));
   ResetObsCache();
      ...
      for(larcid=0;larcid<numLat->na;larcid++){ /* Set correct labels: */
         ...
         if(PhoneMEE){
            for(seg=0;seg<numLat->larcs[larcid].nAlign;seg++){
               ...
               ca = (CorrectArc*) New(&fbInfo->tempStack, sizeof(CorrectArc));
               ...
               for(i=i_start;i<=i_end;i++){
                  CorrectArcList *cal = (CorrectArcList*) New(&fbInfo->tempStack, sizeof(CorrectArcList));
...
void DoMixUpdate(MixPDF *mp, int s, float Lr, float meescale, int t){  
   ...
   if(pMix->time != RealT){
      ...
      if(SavedMixesSize[s] <= indx){
         ...
         NewArray = (MixOcc*) New(&cacheMixoccHeap, sizeof(MixOcc) * NewSize);
         for(n=0;n<indx;n++){
                      ...
                      bool twoDataFiles)
{
   ...
   fbInfo->aInfo = (ArcInfo*) New(&fbInfo->miscStack, sizeof(ArcInfo));
