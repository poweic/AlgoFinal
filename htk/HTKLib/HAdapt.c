...
static int GetVecSizeClass(BaseClass *bclass, int _class)
{
   ILink i;
   MixtureElem *me;

   /* currently does not check consistency of vector sizes */
   i=bclass->ilist[_class];
   me = (MixtureElem *)i->item;
   return VectorSize(me->mpdf->mean);
}

static IntVec GetBlockSize(AdaptXForm *xform, int _class)
{
   ...
   if (strmProj) {
      ...
      blockSize = CreateIntVec(xform->mem,1);
      blockSize[1] = GetVecSizeClass(xform->bclass, _class);
   }
   return blockSize;  
}
...
static void CreateBaseTriMat(MemHeap *x, MixPDF *mp, AdaptXForm *xform, int _class)
{
  TriMat *tm;
  int vsize = VectorSize(mp->mean);
  IntVec blockSize = GetBlockSize(xform, _class);
  RegAcc *regAcc, *ra;
  MixPDF *me;
  BaseClass *bclass;
  ILink i;
  ...
  bclass = xform->bclass;
  for (i=bclass->ilist[_class]; i!=NULL; i=i->next) { 
    if (xform->info->accBTriMat) {
...
void GenXForm(RegNode *node, AdaptXForm *xform, IntVec classes)
{
   ...
   while (classes[_class] == 0) _class++;
   if (strmProj)
      accs = CreateAccStruct(&gstack,xform,xform->hset->vecSize,GetBlockSize(xform, _class));
   else 
      accs = CreateAccStruct(&gstack,xform,node->vsize,GetBlockSize(xform, _class));
