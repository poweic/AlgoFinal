...
LLFInfo *OpenLLF (char *fn)
{
   ...
   if (numLLFs < maxLLFs) {
      ++numLLFs;
      llf = (LLFInfo*) New (&llfHeap, sizeof (LLFInfo));
      llf->next = (LLFInfo*) llfInfo;
      llfInfo = llf;
   }
   else {
      LLFInfo *l;
...
{
   startWord = GetLabId (const_cast<char*>(start ? start : "!SENT_START"), TRUE);
   startLMWord = GetLabId (const_cast<char*>(startLM ? startLM : "<s>"), TRUE);
   endWord = GetLabId (const_cast<char*>(end ? end : "!SENT_END"), TRUE);
   endLMWord = GetLabId (const_cast<char*>(endLM ? endLM : "</s>"), TRUE);
...
static SubLNode *FindAddSubLNode (MemHeap *heap, LNode *ln, LMState lmstate, int *nsln)
{
   ...
   if (!subln) {
      ++*nsln;
      subln = (SubLNode*) New (heap, sizeof (SubLNode));
   ...
   for (i = 0; i < lat->nn; ++i) {
      ...
      for (startSLN = (SubLNode *) ln->hook; startSLN; startSLN = startSLN->next) {
         ...
         for (la = ln->foll; la; la = la->farc) {
            ...
            /* add new subLArc */
            ++nsla;
            sla = (SubLArc*) New (&slaHeap, sizeof (SubLArc));
            sla->lmprob = lmprob;
            sla->end = endSLN;
            sla->la = la;
