...
#define MARK_ALTPATH(a)         (a->prev = (WordendHyp*) ((int)(a->prev) | MARK_ALTPATH_MASK))
#define MARKED_ALTPATH_P(a)     ((int)((a)->prev) & MARK_ALTPATH_MASK)
#define UNMARK_ALTPATH(a)       (a->prev = (WordendHyp*) ((int)(a->prev) & ~MARK_ALTPATH_MASK))

#define GC_ALTPATH_PREV(a)      ((WordendHyp *) ((int)(a)->prev & ~MARK_ALTPATH_MASK))

#ifdef MODALIGN
#define MARK_MODPATH_MASK     0x00000001UL
#define MARK_MODPATH(m)         (m->ln = (LexNode*) ((int)((m)->ln) | MARK_MODPATH_MASK))
#define MARKED_MODPATH_P(m)     ((int)((m)->ln) & MARK_MODPATH_MASK)
#define UNMARK_MODPATH(m)       (m->ln = (LexNode*) ((int)((m)->ln) & ~MARK_MODPATH_MASK))
...
static void SweepPaths (MemHeap *heap)
{
   ...
   for (b = heap->heap; b; b = b->next) {
      for (i = 0, path = (WordendHyp*) b->data; i < b->numElem; 
...
static void SweepAltPaths (MemHeap *heap)
{
   ...
   for (b = heap->heap; b; b = b->next) {
      for (i = 0, path = (AltWordendHyp*) b->data; i < b->numElem; 
...
static void SweepModPaths (MemHeap *heap)
{
   ...
   for (b = heap->heap; b; b = b->next) {
      for (i = 0, path = (ModendHyp*) b->data; i < b->numElem; 
...
void Decoder::GarbageCollectPaths ()
{
   ...
   if (trace&T_GC) {
      ...
      PrintHeapStats (&_dec->weHypHeap);
      PrintHeapStats (&_dec->altweHypHeap);
#ifdef MODALIGN
      if (_dec->modAlign)
        PrintHeapStats (&_dec->modendHypHeap);
#endif
   }
   /*# mark phase */
   ...
   /* sweep phase */
   SweepPaths (&_dec->weHypHeap);
   SweepAltPaths (&_dec->altweHypHeap);
#ifdef MODALIGN
   if (_dec->modAlign)
      SweepModPaths (&_dec->modendHypHeap);
#endif
